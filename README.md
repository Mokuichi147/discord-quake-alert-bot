...
7. 受信ループは受信に専念し、通知処理（地図生成・Discord 送信）は別タスクで行う。緊急地震速報は地震情報とは別の待ち行列で処理し、地図生成に待たされないようにする。
   - 緊急地震速報の待ち行列は、同一地震（`eventId`）につき最新の1件だけを持つ。続報は前の報を差し替えるので、処理が遅れても投稿するのは常に最新の予想になり、古い続報を後から投稿し直すこともない。長さは同時進行している地震の数に収まる。受信から3分を超えた報は投稿しない（揺れが終わった後に「これから強い揺れが来る」と伝えないため）。
