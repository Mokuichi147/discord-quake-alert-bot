...
ファイルの相対パスは起動時の作業ディレクトリ基準です。`NOTIFICATION_CONFIG_FILES` 設定時は、通知設定を各ファイルだけから読み込み、起動用 `.env`・OS の環境変数・他の通知設定からは継承しません。省略した値には従来の既定値を使います。`P2PQUAKE_WS_URL` と `RUST_LOG` は起動用 `.env` / 環境変数だけで指定します。
