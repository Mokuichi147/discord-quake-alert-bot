...
4. 551 は通知条件を満たせば確定通知（観測震度）。津波情報があればタイトルに🌊。震度速報と地震情報は同一地震（発生時刻）で別々に管理し、内容が変われば既存メッセージを差し替える。地震情報は震度速報を包含するため、地震情報を投稿したら**震度速報のメッセージは削除**して重複を残さない（以降の震度速報の続報も投稿しない）。ただし震度速報の発震時刻は暫定値で確定時に修正されることがあり、群発地震では別地震の地震情報と発生時刻が衝突しうる。誤って消さないよう、**対象の都道府県がすべて一致し、かつ最大震度が下がっていない**場合だけ削除する。
