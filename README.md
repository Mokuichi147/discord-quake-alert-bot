...
4. 551 は通知条件を満たせば確定通知（観測震度）。津波情報があればタイトルに🌊。震度速報と地震情報は同一地震（発生時刻）で別々に管理し、内容が変われば既存メッセージを差し替える。地震情報は震度速報を包含するため、地震情報を投稿したら**震度速報のメッセージは削除**して重複を残さない（以降の震度速報の続報も投稿しない）。
