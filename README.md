...
9. 再接続の前に履歴 API を確認し、切れている間に流れた地震情報(551)を拾い直す。緊急地震速報(556)は対象外（揺れる前に知らせる情報なので、終わった後に流さない）。起動直後は基準を記録するだけで、起動前の地震は通知しない（プロセスを再起動した場合、停止中の地震は拾われません）。
10. Discord への送信が一時的な理由（接続エラー・タイムアウト・429・5xx）で失敗した場合は最大3回まで試し直す。
