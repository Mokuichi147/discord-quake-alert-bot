...
- 既定では関東のみ `KANTO_MIN_SCALE`（既定 40 = 震度4）。例えば東北・近畿も震度4で通知したい場合は `TOHOKU_MIN_SCALE=40` `KINKI_MIN_SCALE=40` を設定します。
...
| `<地方>_MIN_SCALE` | 関東のみ`40` | 地方ごとの通知する最小震度スケール（上記「地方区分と環境変数」を参照） |
