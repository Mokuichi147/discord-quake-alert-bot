...
9. 接続した**後**に履歴 API を確認し、切れている間に流れた地震情報(551)・津波予報(552)を拾い直す（購読を先に始めることで、履歴と購読の間に隙間を作らない）。緊急地震速報(556)は対象外（揺れる前に知らせる情報なので、終わった後に流さない）。基準は通知まで終えた分だけ進めるため、送信に失敗した報は次の再接続で拾い直される。起動直後は基準を記録するだけで、起動前の分は通知しない（プロセスを再起動した場合、停止中の分は拾われません）。
...
- **WebSocket は1つの IP につき同時1接続まで**です。2本目の接続要求は `429 Too Many Requests` で拒否され、切断したセッションが解放されるまでにも十数秒かかります（実測）。同じ IP で bot を二重に起動すると、後から起動した方は接続できません。再起動の直後にしばらく 429 が続くのもこのためで、bot 側は10秒間隔で張り直します（テストモード `--test` 系は REST API のみを使うため、この制限には関係しません）。
