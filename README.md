...
7. 受信ループは受信に専念し、通知処理（地図生成・Discord 送信）は別タスクで行う。緊急地震速報は地震情報とは別の待ち行列で処理し、地図生成に待たされないようにする。
...
10. Discord への送信が一時的な理由で失敗した場合は最大3回まで試し直す。ただし新規投稿は、届いたかどうかが曖昧な失敗（タイムアウト・5xx）では試し直さない（二重投稿になり、先に作られたメッセージを続報で差し替えられなくなるため）。メッセージの編集は何度投げても同じなので、曖昧な失敗でも試し直す。
...
- **WebSocket には IP アドレスごとの接続数制限があります。** [公式仕様](https://www.p2pquake.net/develop/json_api_v2/)では `/ws: 2 接続 (IP アドレス毎)` ですが、実測では2本目の握手がすぐ `429 Too Many Requests` になり、切断したセッションが解放されるまでにも十数秒かかりました。いずれにせよ、同じ IP で bot を二重に起動すると後から起動した方は接続できません。再起動の直後にしばらく 429 が続くのもこのためで、bot 側は10秒間隔で張り直します（テストモード `--test` 系は REST API のみを使うため、この制限には関係しません）。
