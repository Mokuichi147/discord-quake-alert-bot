...
7. 受信ループは受信に専念し、通知処理（地図生成・Discord 送信）は別タスクで行う。緊急地震速報は地震情報とは別の待ち行列で処理し、地図生成に待たされないようにする。
   - 緊急地震速報の待ち行列は、同一地震（`eventId`）につき最新の1件だけを持つ。続報は前の報を差し替えるので、処理が遅れても投稿するのは常に最新の予想になり、古い続報を後から投稿し直すこともない。長さは同時進行している地震の数に収まる。受信から3分を超えた報は投稿しない（揺れが終わった後に「これから強い揺れが来る」と伝えないため）。差し替えても「通知条件を満たす報があった」ことは保つので、初報が条件を満たし続報が下方修正だった場合も、引き下げ後の内容で通知する（通知してから引き下げられた場合と同じ結果になる）。送信に失敗した報は、3分以内かつ5回までは積み直して送り直す（履歴から拾い直せないため）。
