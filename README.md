...
7. 受信ループは受信に専念し、通知処理（地図生成・Discord 送信）は別タスクで行う。緊急地震速報は地震情報とは別の待ち行列で処理し、地図生成に待たされないようにする。待ち行列に上限は置かない（溢れて捨てると、履歴から拾い直せない緊急地震速報は永久に失われるため。通知処理はすべて制限時間付きなので、溜まっても必ずはけていく）。
...
10. 通知できなかった報が残っている間、または基準をまだ取れていない間は、接続が続いていても60秒ごとに履歴を見に行く。切断を待たずに再送するための経路で、起動直後の履歴取得に失敗した場合の取り直しも兼ねる。
